...
def fetch_model_output():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM recommendations_caffeine;")
        rows = cursor.fetchall()
        colnames = [desc[0] for desc in cursor.description]
    except Exception as e:
        print(f"查詢 recommendations_caffeine 失敗: {e}")
        return []
    finally:
        cursor.close()
        conn.close()
            ...
            for user_id_sleep, start_time, end_time in sleep_data:
                if user_id == user_id_sleep:
                    ...
                    if not intake_schedule:
                        recommendations.append((user_id, 0, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                    else:
